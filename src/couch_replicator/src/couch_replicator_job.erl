...
terminate(shutdown, #rep_state{} = State0) ->
    ...
    State3 = case do_checkpoint(State1) of {ok, State2} -> State2; Error ->
...
do_init(Job, #{} = JobData) ->
    ...
    case Owner of owner -> ok; not_owner ->
        exit({shutdown, finished})
    end,
...
init_state(#{} = Job, #{} = JobData) ->
    ...
    [SourceLog, TargetLog] = find_and_migrate_logs([Source, Target], Rep, BaseId),
...
do_checkpoint(State) ->
    ...
    {SrcInstanceStartTime, TgtInstanceStartTime} ->
        ...
        StartTime = couch_replicator_utils:rfc1123_local(ReplicationStartTime),
